...
def draw_skeletons(npimg, skeletons, all_keypoint_types, marker='circle', circle_radius=4, alpha=0.7, thickness=-1, stick_width=4, draw_keypoints=True, limbs=[], beta=0.6):
    ...
    :param skeletons: list, a list of keypoint dictionaries, each dict represents a skeleton, like [{'kp_type1': [x, y], ...}, ...]
    ...
    for each_skeleton in skeletons:  # humans is [OrderedDict1, OrderDict2, ...]
        if draw_limbs == True:
            ...
            for pair_order, pair_ids in enumerate(PAIRS):
                ...
                kp1 = each_skeleton[kp_type1][:2]
                kp2 = each_skeleton[kp_type2][:2]
                # cv2.line(npimg_cur, kp1, kp2, CocoColors[pair_order], stick_width)
                cv2.line(npimg_cur, kp1, kp2, [255, 255, 255], stick_width)
                # cv2.line(npimg_cur, kp1, kp2, [255, 0, 0], stick_width)
                # cv2.line(npimg_cur, kp1, kp2, CocoColors[pair_order], stick_width)
                ...
                # cv2.fillConvexPoly(npimg_cur, polygon, CocoColors[pair_order])
        ...
        if draw_keypoints == True:
            ...
            for i in KEYPOINT_IDS:
                ...
                coco_color = CocoColors[i % len(CocoColors)]
                if marker == 'circle':
                    # marker 1: circle, default size 4
                    # if thickness > 0:
                    #     cv2.circle(npimg_cur, center, circle_radius, [255, 255, 255], thickness=thickness+3)
                    cv2.circle(npimg_cur, center, circle_radius, coco_color, thickness=thickness)  # circle_thickness = -1, fill; otherwise does not fill
                    # if thickness < 0:
                    ...
                    cv2.drawMarker(npimg_cur, center, coco_color, cv2.MARKER_TILTED_CROSS, thickness=2, markerSize=circle_radius)
                elif marker == 'cross':
                    # marker 3: cross, default thickness 2, size 8
                    cv2.drawMarker(npimg_cur, center, coco_color, cv2.MARKER_CROSS, thickness=2, markerSize=circle_radius)
                elif marker == 'diamond':
                    # marker 4: diamond, default thickness 2, size 8
                    cv2.drawMarker(npimg_cur, center, coco_color, markerType=cv2.MARKER_DIAMOND, thickness=2, markerSize=circle_radius)
                elif marker == 'triangle_up':
                    # marker 5: triangle up, default thickness 2, size 6
                    cv2.drawMarker(npimg_cur, center, coco_color, markerType=cv2.MARKER_TRIANGLE_UP, thickness=2, markerSize=circle_radius)
                elif marker == 'square':
                    # marker 6: square, default thickness 2, size 6
                    cv2.drawMarker(npimg_cur, center, coco_color, markerType=cv2.MARKER_SQUARE, thickness=2, markerSize=circle_radius)
...
def draw_instance(image_path, keypoints, full_keypoint_types, limbs=[], visible_bounds=None, hightlight_keypoint_types=None, save_root=None, save_prefix="", is_show=True, circle_radius=4, stick_width=4):
    ...
    if hightlight_keypoint_types != None:
        ...
        for kp_type in hightlight_keypoint_types:
            ...
            center = (int(body_part[0]), int(body_part[1]))
            cv2.circle(image_curr, center, 15, CocoColors[KEYPOINT_TYPE_IDS[kp_type]], thickness=-1)  # 15
...
def draw_ellipses(image, ellipses_list, color=[255,255,255], thickness=-1, alpha=1.0):
    '''
    image: numpy BGR image, H x W x 3
    ellipses_list: a list of ellipses needed to draw, each ellipse is [xc, yc, a, b, theta]. theta is in degrees,
    clockwise is positive degree, anti-clockwise is negative degree.
    '''
    npimg_cur = np.copy(image)
    for i, ellipse in enumerate(ellipses_list):
        cx, cy = ellipse[0], ellipse[1]
        center = (int(cx), int(cy))
        semi_major = ellipse[2]
        semi_minor = ellipse[3]
        semi_axes = (int(semi_major+0.5), int(semi_minor+0.5))
        angle = ellipse[4]
        cv2.ellipse(npimg_cur, center, semi_axes, angle=angle, startAngle=0, endAngle=360, color=color, thickness=thickness)
