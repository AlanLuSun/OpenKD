...
def does_det_success(kp_list1, kp_list2, thresh=384*0.05):  # 384*0.01
    thresh_sq = thresh ** 2
    flag = True
...
def save_predictions(supports, support_labels, support_kp_mask, queries, query_labels, query_kp_mask, predictions, episode_generator, episode_num=0,
    ...
    for b in  range(B2):
        anno = episode_generator.queries[b]
        im_id = anno['image_id']
        ...
        kps_gt_dict = {kp_type: kp_tmp[n, :] for n, kp_type in enumerate(episode_generator.support_kp_categories) \
        ...
        kps_pred_dict = {kp_type: predict_kp_tmp[n, :] for n, kp_type in enumerate(episode_generator.support_kp_categories) \
                     if query_kp_mask[b, n] > 0 }
...
def save_heatmaps(queries, query_labels, query_kp_mask, predictions, multi_group_heatmaps, fused_heatmaps,
                  episode_generator, episode_num=0, save_root='output/episode_images/predictions',
                  param_type='CLIP', kp_texts=(), **kwargs):
    ...
    for b in range(B2):
        ...
        for n, kp_type in enumerate(episode_generator.support_kp_categories):
            if query_kp_mask[b, n] > 0:
        ...
        for n in range(N_kps):
            ...
            kp_index = episode_generator.support_kp_categories.index(kp_type)
            for g in range(G):
